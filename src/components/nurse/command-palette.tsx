...
type PaletteItem = {
  ...
  label: string;
  href?: string;
  icon?: (typeof NURSE_NAV)[number]["icon"];
  group: string;
};

const ITEMS: PaletteItem[] = NURSE_NAV.map((n) => ({
  id: n.id,
  label: n.label,
  href: n.href,
  icon: n.icon,
...
export function NurseCommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    ...
    >
      <div
        className="w-full max-w-[520px] overflow-hidden rounded-xl border border-[var(--attio-border)] bg-white shadow-2xl"
      ...
      >
        <div className="flex items-center gap-2 border-b px-3">
          ...
          <input
            ...
            placeholder="Search queue, patients, consent…"
