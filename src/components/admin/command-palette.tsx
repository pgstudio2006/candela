...
import { ADMIN_NAV } from "@/design-system/admin-nav";
...
export function AdminCommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const items = useMemo(
    () => ADMIN_NAV.filter((i) => !q || i.label.toLowerCase().includes(q.toLowerCase())),
    [q],
  );
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter" && items[activeIndex]) { e.preventDefault(); run(items[activeIndex].href); }
    };
    ...
    <div className="fixed inset-0 z-[200] flex items-start justify-center bg-black/25 px-4 pt-[14vh] backdrop-blur-[2px]" onClick={onClose}>
      <div className="w-full max-w-[520px] overflow-hidden rounded-xl border border-[var(--attio-border)] bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 border-b px-3">
          ...
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search admin modules…" className="h-11 flex-1 bg-transparent text-[14px] outline-none" />
        </div>
        <ul className="max-h-80 overflow-y-auto p-1.5">
          {items.map((item, index) => (
            <li key={item.id}>
              <button type="button" className={cn("flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[13px]", index === activeIndex ? "bg-[var(--attio-active)]" : "hover:bg-[var(--attio-surface)]")} onClick={() => run(item.href)}>
                <SidebarIcon icon={item.icon} active={index === activeIndex} />
                {item.label}
              </button>
            </li>
          ))}
