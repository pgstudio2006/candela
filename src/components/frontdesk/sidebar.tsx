...
const SIDEBAR_KEY = "candela-sidebar-collapsed";
...
export function WorkspaceSidebar({
  ...
  return (
    ...
    >
      {/* Header — logo + toggle (Attio / Basepoint) */}
      ...
      <ScrollArea className="min-h-0 flex-1 px-2 py-2">
        ...
        <nav className={cn("space-y-0.5", !collapsed && "mb-3")}>
          {FRONTDESK_NAV.map((item) => {
            const active =
              item.href === "/app/frontdesk"
                ? pathname === item.href
        ...
        {!collapsed && (
          <button
            ...
            className="mb-1 flex w-full items-center gap-1 px-2 text-[10px] font-semibold tracking-[0.08em] text-[var(--attio-text-tertiary)] uppercase"
          >
            {listsOpen ? <ChevronDown className="size-3" /> : <ChevronRight className="size-3" />}
            Lists
          </button>
        )}
        ...
        {(collapsed || listsOpen) && (
          <nav className={cn("space-y-0.5", !collapsed && "mb-3")}>
            {FRONTDESK_LISTS.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                title={collapsed ? item.label : undefined}
                className={cn(
                  "flex items-center rounded-md py-1.5 text-[13px] text-[var(--attio-text-secondary)] hover:bg-[var(--attio-hover)] hover:text-[var(--attio-text)]",
                  collapsed ? "justify-center px-0" : "gap-2.5 px-2",
                )}
              >
        ...
        >
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[10px] text-[var(--attio-text-tertiary)]">Front Desk</p>
            </div>
          )}
          {!collapsed && (
            <>
