...
import { CRM_NAV, CRM_NAV_GROUPS } from "@/design-system/crm-nav";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const SIDEBAR_KEY = "candela-crm-sidebar-collapsed";
...
export function CrmSidebar({
  ...
  const navItems = CRM_NAV.filter((n) => isManager() || !n.managerOnly);
      ...
      <div className={cn("shrink-0 border-b border-[var(--attio-border-subtle)]", collapsed ? "px-1.5 py-2" : "px-2 py-2")}>
          ...
          {collapsed ? (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-52">
                <DropdownMenuItem className="font-medium">CRM Workspace</DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem>{branchName}</DropdownMenuItem>
                <DropdownMenuItem className="text-[var(--attio-text-tertiary)]">{userEmail}</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          )}
          <button
        ...
        {!collapsed && (
          ...
          >
            ...
            <span className="flex-1 text-left">Search leads & actions</span>
          ...
          return (
            <div key={group.id} className="mb-3">
              ...
              <nav className="space-y-0.5">
                {items.map((item) => {
                  const active = item.href === "/app/crm" ? pathname === item.href : pathname.startsWith(item.href);
                  return (
                    <Link
                      key={item.id}
                      href={item.href}
            ...
            <>
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[10px] capitalize text-[var(--attio-text-tertiary)]">
                  {isManager() ? "CRM Manager" : operator?.role ?? "CRM"}
                </p>
              </div>
              <Link href="/app/crm/settings" className="rounded p-1 text-[var(--attio-text-tertiary)] hover:bg-[var(--attio-hover)]">
