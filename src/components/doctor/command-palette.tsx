...
import { Command, Settings, Stethoscope } from "lucide-react";
...
type PaletteItem = {
  ...
  label: string;
  href?: string;
  icon?: (typeof DOCTOR_NAV)[number]["icon"];
  group: string;
  action?: "settings" | "next-patient";
};
...
type DoctorCommandPaletteProps = {
  open: boolean;
  onClose: () => void;
  onOpenSettings?: () => void;
  onStartNext?: () => void;
...
export function DoctorCommandPalette({
  ...
  const runItem = useCallback(
    (item: PaletteItem) => {
      ...
      if (item.action === "next-patient") {
        onStartNext?.();
        return;
      }
      if (item.href) router.push(item.href);
    },
    [onClose, onOpenSettings, onStartNext, router],
  );
      ...
      >
        <div className="flex items-center gap-2 border-b border-[var(--attio-border-subtle)] px-3">
          ...
          <input
            ...
            placeholder="Search patients, consults, screens…"
