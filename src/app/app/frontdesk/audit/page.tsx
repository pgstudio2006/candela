...
import { listFrontdeskAuditLogsAction } from "@/app/actions/clinical-actions";
import { PageChrome } from "@/components/frontdesk/page-chrome";
import { AttioButton, Panel, StatusBadge } from "@/components/frontdesk/ui";
import { useFrontdeskPoll } from "@/hooks/use-frontdesk-poll";
import { useCallback, useEffect, useState } from "react";

type AuditRow = Awaited<ReturnType<typeof listFrontdeskAuditLogsAction>>[number];

export default function FrontdeskAuditPage() {
  useFrontdeskPoll(30_000);
  const [logs, setLogs] = useState<AuditRow[]>([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    const rows = await listFrontdeskAuditLogsAction({ limit: 80 });
    setLogs(rows);
    setLoading(false);
  }, []);
  ...
  return (
    <PageChrome
      ...
      title="Front desk audit log"
      meta="Registration · billing · appointments · patient edits"
      actions={
              ...
              <li key={log.id} className="py-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="text-right">
                    <StatusBadge
                      label={log.severity}
                      variant={log.severity === "critical" ? "warning" : "neutral"}
                    />
