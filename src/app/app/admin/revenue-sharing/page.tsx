...
export default function AdminRevenueSharingPage() {
  ...
  return (
    <PageChrome breadcrumbs={[{ label: "Admin", href: "/app/admin" }, { label: "Revenue sharing" }]} title="Doctor revenue sharing" meta="Custom policies · department scope · settlement simulator">
      ...
      {revenuePolicies.length === 0 ? (
        <Panel title="No revenue policies configured">
          <p className="text-[13px] text-[var(--attio-text-tertiary)]">
            No revenue sharing policies have been created yet. Add a policy to enable doctor revenue sharing.
          </p>
        </Panel>
      ) : (
        <>
